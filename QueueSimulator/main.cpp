...
public:
	...
	bool isFree(const int time, const int serviceTime) {
		return time >= endServiceTime && time + serviceTime < 43200;
	}
...
int main(int argc, const char * argv[]) {
  ...
  // initalization
	...
	//std::cout << i << ' ' << j << ' ' << ' ' << tenth << ' ' << fifty << ' ' << ninty << '\n';
	std::cout << i << ' ' << j << ' ' << ' ' << Mtenth << ' ' << Mfifty << ' ' << Mninty << '\n';
	//std::cout << Mtenth << ' ' << Mfifty << ' ' << Mninty << '\n';
